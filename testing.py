...
def test_simple_check_game_ended() -> None:
    """Test GameBoard.check_game_ended on the docstring example"""
    b = GameBoard(3, 2)
    Raccoon(b, 1, 0)
    Player(b, 0, 0)
    RecyclingBin(b, 1, 1)
    assert b.check_game_ended() is None
    assert not b.ended
    RecyclingBin(b, 2, 0)
    score = b.check_game_ended()
    assert b.ended
    assert score == 11
