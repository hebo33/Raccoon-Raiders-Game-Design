...
class RaccoonRaiders:
    ...
    height: int
    square_size: int
    _board: implementation.GameBoard
    ...
    def __init__(self, w: int, h: int, board_string: str = '') -> None:
        ...
        the board.
        """
        self._board = implementation.GameBoard(w, h)
...
# in order to work.
def populate_board(board: implementation.GameBoard, num_raccoons: int,
    ...
    >>> b = implementation.GameBoard(3, 1)
    >>> populate_board(b,1,0,1)
    >>> str(b) in ['PRB', 'PBR', 'PSB', 'PBS']
    True
    """

    implementation.Player(board, 0, 0)
    ...
    for _ in range(num_raccoons):
        ...
        if random() <= FRACTION_SMART:
            implementation.SmartRaccoon(board, x, y)
        else:
            implementation.Raccoon(board, x, y)

    for _ in range(num_cans):
        x, y = availables.pop()
        locked = random() <= FRACTION_LOCKED
        implementation.GarbageCan(board, x, y, locked)

    for _ in range(num_bins):
        x, y = availables.pop()
        implementation.RecyclingBin(board, x, y)
